#!/usr/bin/env python3
# ===================== train_estimator_multi_npz_all_mse_tfmr_fast.py =====================
...
USER_CFG = dict(
    ...
    # dataloader
    num_workers=8,          # <<< speed: increase workers
    prefetch_factor=8,      # <<< speed: more prefetch
    persistent_workers=True,

    ckpt=str(THIS_DIR / "newBandit_multi_0.2_150_10mps_mSe_lr1e-3_W20_tfmr_global_FAST.pt"),
    device=("cuda:0" if torch.cuda.is_available() else "cpu"),

    # speed toggles
    use_amp=True,           # <<< speed: AMP on A100
    compile_model=False,    # optional: torch.compile
)
...
def parse_args() -> dict:
    ...
    p.add_argument("--device", type=str)  # allow "cuda:0", "cuda", "cpu"
    p.add_argument("--num_workers", type=int)
    p.add_argument("--prefetch_factor", type=int)
    p.add_argument("--persistent_workers", type=int, choices=[0,1])
    ...
    p.add_argument("--use_amp", type=int, choices=[0,1])
    p.add_argument("--compile_model", type=int, choices=[0,1])
    ...
    if "compile_model" in args:
        args["compile_model"] = bool(args["compile_model"])
    return args
...
class MasksOnlyDatasetNext(Dataset):
    ...
    SPEED: convert numpy -> torch once in __init__ (avoid per-sample torch.tensor)
    """
    def __init__(self, npz_path: str, W: int, H: int = 1,
        ...
        if cap_vec is not None:
            cap = np.asarray(cap_vec, dtype=np.float32)
            assert cap.shape == (N,)
        ...
        # store as torch tensors once (CPU)
        self.rm_t    = torch.from_numpy(rm)
        self.sm_t    = torch.from_numpy(sm)
        self.mask_t  = torch.from_numpy(m)
        self.rates_t = torch.from_numpy(rates)
    ...
    def __getitem__(self, idx: int):
        ...
        RM_hist = self.rm_t[s:e]        # (W,N)
        SM_hist = self.sm_t[s:e]        # (W,N)
        M_hist  = self.mask_t[s:e]      # (W,N)
        R_tp1   = self.rates_t[tp1]     # (N,)
        A_t     = self.mask_t[t]        # (N,)
        ...
        R_tp1 = torch.nan_to_num(R_tp1)
        A_t = torch.nan_to_num(A_t)
        return X, R_tp1, A_t
...
class RateEstimatorV2(nn.Module):
    """
    (B,W,N,F) ->
      scaler -> concat -> TemporalConv (B,N,z) -> AntennaTransformer (B,N,z) ->
      concat raw_mean -> head -> cap
    """
    ...
    def forward(self, X_hist):  # (B,W,N,F)
        ...
        z = self.temporal(x_perm)                             # (B,N,z)
        z = self.interact(z, groups=self.groups)              # (B,N,z)

        raw_mean = torch.nan_to_num(X_hist.mean(dim=1))       # (B,N,F)
        h = torch.cat([z, raw_mean], dim=-1)                  # (B,N,z+F)
...
class TrainCfg:
    ...
    ckpt: str = str(THIS_DIR / "rate_estimator_tfmr_fast.pt")
    seed: int = 2025
...
def print_device_info(cfg: TrainCfg):
...
def main():
    ...
    def make_concat(file_list: List[Path]) -> ConcatDataset:
        dss = []
        for p in file_list:
            dss.append(MasksOnlyDatasetNext(str(p), W=cfg.W, H=cfg.H, cap_vec=tuple(cap_vec)))
    ...
    model = RateEstimatorV2(
        ...
        use_cap=True, use_softcap=False, softcap_sharp=1.0,
        groups=None,
        num_heads=cfg.num_heads,
        attn_mode=cfg.attn_mode,
        attn_bias=cfg.attn_bias,
        use_ant_id=cfg.use_ant_id,
    ...
    # quick throughput probe (optional)
    # t0 = time.time()
    # X0, R0, _ = next(iter(train_loader))
    # print("[DEBUG] first batch load sec =", time.time() - t0)
            ...
            if not first_debug_printed:
                with torch.no_grad():
                    print("[DEBUG] X finite:", torch.isfinite(X).all().item(),
                          ...
                          "shape:", tuple(X.shape))
                    print("[DEVICE] X.device =", X.device, "R.device =", R.device)
                    print("[DEVICE] model param device =", next(model.parameters()).device)
