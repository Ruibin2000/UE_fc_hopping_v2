# ===================== train_estimator_multi_npz_all_mse_lstm.py =====================
...
USER_CFG = dict(
    ...
    # cap (optional). If None, will estimate by max over train files
...
class MasksOnlyDatasetNext(Dataset):
    ...
    def __init__(self, npz_path: str, W: int, H: int = 1,
        ...
        if cap_vec is not None:
            ...
            assert cap.shape == (N,), f"cap_vec shape {cap.shape} != ({N},)"
    ...
    def __getitem__(self, idx: int):
        ...
        RM_hist = torch.tensor(self.rm[s:e],   dtype=torch.float32)   # (W,N)
        SM_hist = torch.tensor(self.sm[s:e],   dtype=torch.float32)   # (W,N)
        M_hist  = torch.tensor(self.mask[s:e], dtype=torch.float32)   # (W,N)

        R_tp1   = torch.tensor(self.rates[tp1], dtype=torch.float32)  # (N,)
        A_t     = torch.tensor(self.mask[t],    dtype=torch.float32)  # (N,)
...
class RateEstimatorLSTM(nn.Module):
    """
    Per-antenna shared LSTM baseline
    Input : X_hist (B, W, N, F)
    Output: {"R_hat": (B, N)}
    """
    def __init__(self,
                 num_antennas: int,
        ...
        # match TFMR preprocessing fairness
    ...
    def forward(self, X_hist: torch.Tensor):
        ...
        # (B,W,N,F) -> (B,N,W,F) -> (B*N,W,F)
        x = X_scaled.permute(0, 2, 1, 3).contiguous().view(B * N, W, F)

        _, (h_n, _) = self.lstm(x)
        h_last = h_n[-1]  # (B*N, hidden)

        raw_mean = torch.nan_to_num(X_hist.mean(dim=1)).contiguous().view(B * N, F)  # (B*N,F)

        h = torch.cat([h_last, raw_mean], dim=-1)  # (B*N, hidden+F)
        y = self.head(h).view(B, N)                # (B,N)
...
@dataclass
class TrainCfg:
    data_dir: str = str(THIS_DIR / "dataset" / "newBandit_npz_0.2_150_10mps")
    pattern_prefix: str = "bandit_1_"
    pattern_suffix: str = "_10mps.npz"
    ...
    ckpt: str = str(THIS_DIR / "rate_estimator_allmse_LSTM_fp32.pt")
    seed: int = 2025
...
def save_ckpt(path: str, model: nn.Module, opt: torch.optim.Optimizer, epoch: int,
    ...
    payload = {
        ...
        "hidden": int(getattr(model, "hidden", cfg_dict.get("hidden", 128))),
        "num_layers": int(getattr(model, "num_layers", cfg_dict.get("num_layers", 2))),
...
def main():
    ...
    # write test file list (same behavior as TFMR script)
    ...
    model = RateEstimatorLSTM(
        ...
        dropout=cfg.dropout,
        use_cap=True,
        use_softcap=cfg.use_softcap,
        softcap_sharp=cfg.softcap_sharp,
    ).to(cfg.device)
    ...
    for epoch in trange(1, cfg.epochs + 1, desc="Epochs"):
        ...
        if last_val_mse < best_val - 1e-9:
            ...
            save_ckpt(
                cfg.ckpt, model, opt, epoch, best_val,
                ds_N=N, feat_dim=3, cap_vec=cap_vec, cfg_dict=dict(vars(cfg))
            )
            saved_once = True
    ...
    if not saved_once:
        ...
        save_ckpt(
            cfg.ckpt, model, opt, cfg.epochs, fallback_val,
            ds_N=N, feat_dim=3, cap_vec=cap_vec, cfg_dict=dict(vars(cfg))
        )
