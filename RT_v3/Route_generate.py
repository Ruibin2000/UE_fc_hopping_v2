...
# ============================================================
# 0) ENV MUST BE SET BEFORE IMPORTING TENSORFLOW
# ============================================================
import os

# Set GPU visibility BEFORE importing TF.
# Use "" to disable GPU entirely.
...
# 6) Scene loader
# ============================================================
def load_and_prepare_scene(scene_name="nyu_tandon"):
    scenes = list_scenes()
    if scene_name not in scenes:
        raise ValueError(f"Scene '{scene_name}' not found. Available: {scenes}")

    scene_path, map_data = get_scene(scene_name)
    scene = load_scene(scene_path, merge_shapes=True)

    # Example material tweak
    ...
    # Remove wet ground if exists
    if "itu_wet_ground" in scene.objects:
...
def main():
    ...
    scene, map_data = load_and_prepare_scene(scene_name)
