...
def bandit_link_single_ue(capacity, epsilon=0.2, alpha=None, seed=None, verbose=False):
    ...
    for t in range(1, n_sample):
        ...
        if rng.random() < epsilon:
            ...
            #   - update all observed links
            #   - choose the best observed link as serving link
            # =============================================================
            explore_flag[t] = True
...
def evaluate_bandit_on_dataset(npz_files, epsilon, alpha=None, base_seed=42, logger=None):
    ...
    for route_idx, npz_path in enumerate(
        tqdm(npz_files, desc=f"bandit epsilon={epsilon:.4f}", ncols=100)
    ):
    ...
    msg = (
        f"[bandit epsilon={overall['epsilon']:.6f}] "
        f"inv_epsilon={overall['inv_epsilon']:.6f}, "
        f"alpha={overall['alpha']}, "
...
def sweep_bandit_epsilons(npz_files, epsilon_list, alpha=None, base_seed=42, logger=None):
    """
...
def plot_inv_epsilon_vs_avg_capacity(results, save_path):
    ...
    plt.title("Bandit Link: 1/epsilon vs average selected capacity")
...
def plot_epsilon_vs_avg_capacity(results, save_path):
    x = [r["epsilon"] for r in results]
    y = [r["avg_selected_capacity"] for r in results]
    ...
    plt.xlabel("epsilon")
    plt.ylabel("Average selected capacity")
    plt.title("Bandit Link: epsilon vs average selected capacity")
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
...
def main():
    ...
    dataset_dir = Path(f"../../dataset/CQI_Mobility_medium_R_2_region_2")
    ...
    experiment_tag = f"bandit_link_semi_bandit_R_2_alpha_{alpha}"

    # dense sweep
    epsilon_list = np.logspace(0, -2, 20)   # 1 -> 0.01, 20 points
    ...
    log_path = os.path.join(out_root, f"results_bandit_link_{timestamp}.log")
    npz_save_path = os.path.join(out_root, f"bandit_link_sweep_summary_{timestamp}.npz")
    fig_inv_path = os.path.join(out_root, f"inv_epsilon_vs_avg_capacity_{timestamp}.png")
    fig_eps_path = os.path.join(out_root, f"epsilon_vs_avg_capacity_{timestamp}.png")
    ...
    print(f"Figure (1/epsilon): {fig_inv_path}")
    print(f"Figure (epsilon): {fig_eps_path}")
    print(f"alpha: {alpha}")
    print("############################################################")
    ...
    plot_inv_epsilon_vs_avg_capacity(results, fig_inv_path)
    plot_epsilon_vs_avg_capacity(results, fig_eps_path)

    logger.info(f"Saved bandit summary npz: {npz_save_path}")
    logger.info(f"Saved figure: {fig_inv_path}")
    logger.info(f"Saved figure: {fig_eps_path}")

    print(f"Saved bandit summary npz: {npz_save_path}")
    print(f"Saved figure: {fig_inv_path}")
    print(f"Saved figure: {fig_eps_path}")
